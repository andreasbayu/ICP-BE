...
# docker run -v ./graphs:/ors-core/data/graphs -v ./elevation_cache:/ors-core/data/elevation_cache -v ./logs/ors:/var/log/ors -v ./logs/tomcat:/usr/local/tomcat/logs -v ./conf:/ors-conf -v ./yogyakarta.pbf:/ors-core/data/osm_file.pbf --name ors-app -u ${ORS_UID:-0}:${ORS_GID:-0} -p 8080:8080 -p 9001:9001 -e BUILD_GRAPHS=True -e JAVA_OPTS=-Djava.awt.headless=true -server -XX:TargetSurvivorRatio=75 -XX:SurvivorRatio=64 -XX:MaxTenuringThreshold=3 -XX:+UseG1GC -XX:+ScavengeBeforeFullGC -XX:ParallelGCThreads=2 -Xmx1g -Xmx1g -e CATALINA_OPTS=-Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.port=9001 -Dcom.sun.management.jmxremote.rmi.port=9001 -Dcom.sun.management.jmxremote.authenticate=false -Dcom.sun.management.jmxremote.ssl=false -Djava.rmi.server.hostname=localhost openrouteservice-yogyakarta/openrouteservice 
